...
# Full local same-sample evaluation for the two indoor panoramic-depth datasets:
#   Matterport3D + Stanford2D3D
#
# It builds a full manifest from local files, audits that RGB/depth are panorama
# shaped, then compares:
...
MANIFEST="${MANIFEST:-panorama_depth_manifest_mp_s2d3d_full.csv}"
AUDIT_DIR="${AUDIT_DIR:-panorama_depth_manifest_mp_s2d3d_full_audit}"

DIRECT_METHOD="${DIRECT_METHOD:-depth_anything3}"
DIRECT_LABEL="${DIRECT_LABEL:-DA3-Direct-Full-MP-S2D3D}"
DIRECT_OUTPUT_DIR="${DIRECT_OUTPUT_DIR:-panorama_depth_eval_da3_direct_mp_s2d3d_full}"

FUSION_INNER_DEPTH_PREDICTOR="${FUSION_INNER_DEPTH_PREDICTOR:-depth_anything3}"
FUSION_LABEL="${FUSION_LABEL:-DreamScene360-PanoGeo-DA3-P240-Full-MP-S2D3D}"
FUSION_DIR="${FUSION_DIR:-panorama_depth_eval_dreamscene360_pano_geo_da3_p240_mp_s2d3d_full}"
COMPARE_OUTPUT_DIR="${COMPARE_OUTPUT_DIR:-panorama_depth_compare_da3_p240_fusion_vs_da3_direct_mp_s2d3d_full}"
...
echo "[stage] build full manifest: $MANIFEST"
...
echo "[stage] audit full manifest"
...
echo "[stage] run full direct-vs-fusion comparison"
DATASETS="$DATASETS" \
